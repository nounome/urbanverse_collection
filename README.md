...
本次打包检查：15 项轻量测试通过；独立目录重新生成 Scene02 50 m 路线；从 Git 导出到另一临时目录并重定位后，12 场景地图/可行区配置检查及采集入口 `--help` 通过。检查借用现有 Python 环境，未复制环境或资产，不等于新机安装/RTX 验收。没有执行下载、GPU 转换或正式数据生成。

2026-09-09 独立仓库运行检查：复用本机已有环境与资产，修复了 `smoke` 关闭相机却仍开启“仅采集时渲染”的入口冲突；该修复已保留。12 场景均实际运行满 3 秒，Go2 位移约 0.345–0.442 m，非足部接触计数均为 0。Scene01/02、Scene04–12 验收通过且退出码为 0；Scene03 逐辆生成汽车，3 秒内只生成 1/7 辆，未满足“所有汽车至少生成一次”的检查，退出码为 2，并非仿真崩溃。该短测验收条件尚未调整，不能称为 12 场景全部验收通过。测试产物不随仓库交付；此结果不代表新机安装、三相机或全路线采集通过。
...
再次精简时移除了不被运行代码读取的建图来源/语义统计报告、未使用的 Kit/wrapper 配置及旧下载模块；规划所需的真实障碍清单、四类地面掩膜、可行区和主体路线仍保留。部分共用模块含旧功能分支，但仍被当前采集器导入，不按文件名直接删除。
